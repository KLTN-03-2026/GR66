...
export default function ToursPage() {
  ...
  const handleToggleStatus = async (id: string) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/tours/status/${id}`, {
        method: "PATCH",
        headers: {
  ...
  const validateTourForm = () => {
    ...
    if (tourForm.image.length < 5) {
      addError("image", "Số ảnh hiện tại không đủ 5");
    }
    if (!tourForm.name.trim()) addError("name", "Vui lòng nhập Tên tour");
    if (!tourForm.location.trim()) addError("location", "Vui lòng nhập Địa điểm");
    if (!tourForm.duration.trim()) addError("duration", "Vui lòng nhập Thời lượng tour");
    if (!tourForm.description.trim()) addError("description", "Vui lòng nhập Mô tả tour");
    if (!tourForm.adultPrice.trim()) addError("adultPrice", "Vui lòng nhập Giá người lớn");
    if (!tourForm.childPrice.trim()) addError("childPrice", "Vui lòng nhập Giá trẻ em");
    if (!tourForm.highlight.trim()) addError("highlight", "Vui lòng nhập Điểm nổi bật");
    if (!tourForm.itinerary.trim()) addError("itinerary", "Vui lòng nhập Lộ trình");
    if (!tourForm.includedPolicy.trim()) {
      addError("includedPolicy", "Vui lòng nhập Chi tiết dịch vụ đi kèm");
    }
    if (!tourForm.serviceTerms.trim()) {
      addError("serviceTerms", "Vui lòng nhập Điều khoản dịch vụ");
    }

    if (!tourForm.adultPrice.trim()) {
      addError("adultPrice", "Vui lòng nhập Giá người lớn");
    } else if (Number(tourForm.adultPrice) <= 0) {
      addError("adultPrice", "Giá người lớn phải lớn hơn 0");
    }

    if (!tourForm.childPrice.trim()) {
      addError("childPrice", "Vui lòng nhập Giá trẻ em");
    } else if (Number(tourForm.childPrice) < 0) {
      addError("childPrice", "Giá trẻ em không được nhỏ hơn 0");
    }
    ...
    if (invalidScheduleIndex !== -1) {
      const schedule = tourForm.schedules[invalidScheduleIndex];
      const scheduleNumber = invalidScheduleIndex + 1;

      if (!schedule.startDate.trim()) {
        addError("schedules", `Vui lòng nhập Ngày khởi hành ở lịch ${scheduleNumber}`);
      } else if (!schedule.endDate.trim()) {
        addError("schedules", `Vui lòng nhập Ngày kết thúc ở lịch ${scheduleNumber}`);
      } else if (!schedule.seatCount.trim()) {
        addError("schedules", `Vui lòng nhập Số chỗ ở lịch ${scheduleNumber}`);
      } else if (!schedule.remaining.trim()) {
        addError("schedules", `Vui lòng nhập Số chỗ còn lại ở lịch ${scheduleNumber}`);
      }
    } else {
      ...
      if (invalidDateIndex !== -1) {
        const schedule = tourForm.schedules[invalidDateIndex];
        const scheduleNumber = invalidDateIndex + 1;

        if (schedule.startDate < today) {
          addError("schedules", `Ngày khởi hành ở lịch ${scheduleNumber} không được nhỏ hơn ngày hiện tại`);
        } else {
          addError("schedules", `Ngày kết thúc ở lịch ${scheduleNumber} không được nhỏ hơn ngày khởi hành`);
        }
      }
    }

    if (tourForm.image.length > 5) {
      addError("image", "Chỉ được tối đa 5 ảnh");
    }

    if (editingTourId === null && tourForm.image.length === 0) {
      addError("image", "Vui lòng tải ảnh tour");
    }
    ...
    if (hasInvalidService) {
      ...
      if (invalidIndex !== -1) {
        const service = tourForm.extraServices[invalidIndex];
        const serviceNumber = invalidIndex + 1;

        if (!service.serviceName.trim()) {
          addError("extraServices", `Vui lòng nhập Tên dịch vụ thêm ${serviceNumber}`);
        } else if (!service.serviceType.trim()) {
          addError("extraServices", `Vui lòng nhập Loại dịch vụ thêm ${serviceNumber}`);
        } else if (!service.includedContent.trim()) {
          addError("extraServices", `Vui lòng nhập Nội dung bao gồm của dịch vụ thêm ${serviceNumber}`);
        } else if (!service.excludedContent.trim()) {
          addError("extraServices", `Vui lòng nhập Nội dung không bao gồm của dịch vụ thêm ${serviceNumber}`);
        } else if (!service.terms.trim()) {
          addError("extraServices", `Vui lòng nhập Điều khoản của dịch vụ thêm ${serviceNumber}`);
        } else if (!service.adultPrice.trim()) {
          addError("extraServices", `Vui lòng nhập Giá người lớn của dịch vụ thêm ${serviceNumber}`);
        } else if (!service.childPrice.trim()) {
          addError("extraServices", `Vui lòng nhập Giá trẻ em của dịch vụ thêm ${serviceNumber}`);
        }
      }
    }
  ...
  const createTourFormData = () => {
    ...
    const schedulesPayload = tourForm.schedules.map((schedule) => ({
      ngaykhoihanh: schedule.startDate,
      ngayketthuc: schedule.endDate,
      Socho: Number(schedule.seatCount) || 0,
      Conlai: Number(schedule.remaining) || Number(schedule.seatCount) || 0,
    }));

    // Gửi cả 2 key để khớp BE đang dùng tourSchedules hoặc schedules
    formData.append("tourSchedules", JSON.stringify(schedulesPayload));
  ...
  const handleSubmitTour = async () => {
    ...
    if (imageFiles.length < 5) {
      ...
      alert("Vui lòng tải tối thiểu 5 ảnh tour");
      return;
    }
          ...
          <div className="rounded-[12px] border border-[#BFC5CD] bg-[#F9F9F9] p-5 md:p-7">
            <div className="max-w-[980px]">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <FieldLabel text="Mô tả của tour" />
                  <RoundedTextarea
                ...
                <div>
                  <FieldLabel text="Thời lượng của tour" />
                  <RoundedInput
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div>
                    <FieldLabel text="Giá tour người lớn" />
                    <RoundedInput
                      type="text"
                      inputMode="numeric"
                  ...
                  <div>
                    <FieldLabel text="Giá tour trẻ em" />
                    <RoundedInput
                      type="text"
                      inputMode="numeric"
                  ...
                  <div className="space-y-4">
                    {tourForm.schedules.map((schedule, index) => (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                          <div>
                            <FieldLabel text="Ngày khởi hành" />
                            <DateInput
                ...
                <div>
                  <FieldLabel text="Điểm nổi bật của tour" />
                  <RoundedTextarea
                ...
                <div>
                  <FieldLabel text="Lộ trình của tour" />
                  <RoundedTextarea
              ...
              <div className="space-y-4">
                <div>
                  <FieldLabel text="Chi tiết dịch vụ đi kèm tour" />
                  <RoundedTextarea
                ...
                <div>
                  <FieldLabel text="Điều khoản dịch vụ" />
                  <RoundedTextarea
