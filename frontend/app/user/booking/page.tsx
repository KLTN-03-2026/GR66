...
function BookingContent() {
  //Lấy đầy đủ các giá trị từ useBookingSubmit
  const {
    handleBookingClick,
    pendingModal,
    setPendingModal,
  ...
  const { totalReviews, averageRating, visibleReviews, handleLoadMore } =
    useReview(reviews);
  ...
  const { total } = useMemo(() => {
    return handleBooking({
      services,
      selectedExtras: confirmedExtras,
      adultCount: adult,
      childCount: child,
      price,
    });
                      ...
                      <div className="flex items-center gap-4 shrink-0 ml-auto">
                        {!isExpanded && (
                          <p className="font-semibold text-xl text-gray-900 whitespace-nowrap">
                            đ {(service.giaApDungNguoiLon ?? 0).toLocaleString()}
                          </p>
                        )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl">
            ...
            <p className="text-gray-600 mb-2">
              Đơn đặt tour của bạn sẽ hết hạn sau:
            </p>
            <p className="text-2xl font-bold text-red-500 mb-6">
              {Math.floor(pendingModal.secondsLeft / 60)} phút{" "}
              {pendingModal.secondsLeft % 60} giây
            </p>
            <p className="text-sm text-gray-500 mb-6">
              Bạn muốn tiếp tục thanh toán hay huỷ đơn cũ và đặt lại?
            </p>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-xl font-semibold transition-all active:scale-95"
              >
                Tiếp tục thanh toán
              </button>
              <button
                ...
                className="w-full border-2 border-red-400 text-red-500 hover:bg-red-50 py-3 rounded-xl font-semibold transition-all active:scale-95"
              >
                Huỷ đơn cũ & đặt lại
              </button>
              <button
